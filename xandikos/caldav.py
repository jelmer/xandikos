...
class CalendarAttachmentPostMethod(webdav.Method):
    ...
    async def handle(self, request, environ, app):
        ...
        # Check if this resource supports managed attachments
        supports_attachments = getattr(resource, "supports_managed_attachments", None)
        if supports_attachments is None or not supports_attachments():
            return webdav._send_simple_dav_error(
                request,
                "403 Forbidden",
                error=ET.Element("{%s}supported-feature" % NAMESPACE),
                description="Calendar does not support managed attachments",
            )
        ...
        resource_supports = getattr(resource, "supports_managed_attachments", None)
        if calendar is None and resource_supports is not None and resource_supports():
            calendar = resource
